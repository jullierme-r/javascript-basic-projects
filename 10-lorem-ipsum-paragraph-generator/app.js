...
form.addEventListener("submit", (e) => {
  ...
  if(num <= 0 || num > 9) { //if the input number is less or equal to 0 or bigger than 9, generates one randomly paragraph
    num = Math.floor(Math.random() * 9);
    paragraphs = text[num];
  } else { //otherwise generates a correspondent number of paragraphs
    paragraphs = generateParagraphs(text, num);
  }
  updateScreen(paragraphs);
});
//return the portion of the array that will be displayed
function generateParagraphs(array, amount) {
  return array.slice(0, amount);
}
//displays the paragraphs on the screen
function updateScreen(array){
  array.forEach((item) => {
    article.innerHTML += `<p class="result">${item}</p>`;
  });
}